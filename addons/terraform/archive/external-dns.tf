    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            "${data.terraform_remote_state.eks.outputs.aws_iam_openid_connect_provider.extract_from_arn}:sub" : "system:serviceaccount:default:external-dns"
          }
        }
      },
    ]
...
resource "helm_release" "external_dns" {
  ...
  set {
    # you need to add a prefix to your registry entry because aws does not allow TXT and CNAME records with same name
    name  = "extraArgs[0]"
    value = "--txt-prefix=txt-"
  }
